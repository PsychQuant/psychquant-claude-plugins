...
**相似度高不等於是同一個東西。** 提高信心的方法不是把一個訊號的門檻拉高，而是**再走一條路徑不同的檢核**——路徑不同，錯誤才不會一致地重複。

以下五種是**不同的路徑**，不是同一種的五個變體。手上的判斷愈貴（會寫入、會刪除、會影響歸戶），就要走愈多條。

#### 3a. 同一筆記錄的多個欄位合取

最基本的一條。work 的例子（實測有效）：
...
拿到 DOI 之後，用**那個 DOI** 回查，比對回來的標題／期刊／年份。方向與查詢階段相反，所以「查詢時選錯候選」不會一致地重複。實測 97 筆抓出 1 筆。

#### 3c. 另一個獨立的提供者

同一個問題問兩個**資料來源不同**的 API。實測：Crossref 與 Europe PMC 對 `10.2188/jea.je20240034` 的七位作者姓名逐字一致——那個一致本身就是證據，遠強於任一方單獨的回答。

不一致時**不要選一個相信**，那是要記下來的歧異。

#### 3d. 同一實體的其他記錄

判斷一筆記錄時，看**同一個人／同一份刊物的其他記錄**怎麼說。

實測：一篇 2026 年的論文出現在某機構的成果清單裡，但論文本身的機構寫的是另一所大學。查該作者的 32 篇著作歷程，發現他 **2010 年**掛該機構、**2013 年後**都在另一所——收錄理由因此站不住。這條路徑用的是**時間軸**，前三條都用不到它。

#### 3e. 手上這份來源檔的其他列

**最容易被忽略、成本最低的一條。** 匯出檔常常自己就含著校正材料。

實測：WoS 匯出把某篇的第七位作者寫成 `Ablm, Yen-Tsung Huang`（`Ablm` 不是姓）。同一份匯出的 **23 列之後**有該篇的更正啟事記錄，作者欄寫的是正確的 `Huang, Yen-Tsung`。**答案一直在檔案裡，只是沒人往下讀。**

同一組還揭露另一處歧異（`Lai, Tai-Hsuan` vs `Lai, Tai-Shuan`），那是靠 3c 判定的。

#### 反例檢核：問「如果這個結論是錯的，會長什麼樣」

比多走一條路更有力的，是**去找推翻它的證據**。

實測：機構字串含 `Institute of Statistical Science` 看似足以判定隸屬。反過來問「如果他不是所內人員，字串會長什麼樣」，就會注意到 `Bioinformatics Program, Institute of Statistical Science, Taiwan International Graduate Program` 裡那個機構名是**學程主辦單位**，而該作者的 email domain 與主機構都指向另一個所。**同一條字串，兩種讀法，只有反例檢核分得出來。**
