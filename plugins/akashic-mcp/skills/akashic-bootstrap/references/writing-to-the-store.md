...
在 store 之外用字串拼 YAML 寫檔，短期看起來能用。實際後果（Akashic-Library#69 的實證）：同一份資料出現兩種位元組表示，而且下一次任何正規寫入碰到那些檔案會靜默重排——產生看起來像資料變更、實際只是排版的 diff，review 的人分不出來。
