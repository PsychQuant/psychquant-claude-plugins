...
#### A. 如果 `.claude/issue-driven-dev.local.json` 已存在
...
無論 C1/C2/C3 選了什麼，都把結果寫入 `.claude/issue-driven-dev.local.json`：
...
直接編輯 `.claude/issue-driven-dev.local.json`，或刪掉讓 skill 重新問一次。
