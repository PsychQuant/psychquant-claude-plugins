...
1. **Step 0.7 Detect** — `--resume-spectra="<topic>"` flag、`--source` 含 `spectra-discuss`、`spectra list` 有 in-flight、`.claude/state/idd-bridge.json` 存在 — 任一觸發 `SPECTRA_BRIDGE_ACTIVE=1`
2. **Step N-1 寫 bookmark** — `.claude/state/idd-bridge.json` 含 topic / issue / open_questions / next_step_hint
