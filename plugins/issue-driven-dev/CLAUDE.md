...
| Skill | 防止的失敗 | 用途 |
|-------|-----------|------|
| `idd-all` | 手動跑 5 個 idd-* 太繁瑣、容易漏步驟、main 上 commit 沒 review checkpoint | 一鍵跑完 issue→diagnose→implement→verify，**強制 PR path**（覆蓋 `pr_policy` config），停在 verified 等人 merge |
