...
當前版本：**v3.13.1**（Plugin shell + Binary 同步）

Office.js OOXML Roadmap **P0 100% 完成**（Umbrella issue [#43](https://github.com/PsychQuant/che-word-mcp/issues/43)）。Latest milestone v3.13.1 ship `document.xml` lossless round-trip + tool-mediated wrapper edits 修正（closes [#56](https://github.com/PsychQuant/che-word-mcp/issues/56) P0）— 保留所有 source `xmlns:*` + `mc:Ignorable`，解析 `<w:bookmarkStart>` / `<w:hyperlink>` / `<w:fldSimple>` / `<mc:AlternateContent>`，sort-by-position Writer emit，`replace_text` 走 wrapper-internal runs（無 silent failure）。v3.13.1 是 v3.13.0 的 hot-fix（pPr double-emission silent regression）— 詳見 [CHANGELOG](https://github.com/PsychQuant/che-word-mcp/blob/main/CHANGELOG.md)。v3.12.0 ship programmatic Track Changes 寫側（[#45](https://github.com/PsychQuant/che-word-mcp/issues/45)）。
...
- **OOXML 引擎**: [`ooxml-swift v0.19.1`](https://github.com/PsychQuant/ooxml-swift)（preserve-by-default + dirty tracking + revision generation + `document.xml` lossless round-trip）
...
- **Plugin shell**: v3.13.1
- **Binary**: v3.13.1（`CheWordMCP`）
...
兩者獨立但本次 v3.13.1 同步。Plugin shell（marketplace 端，含 SKILL.md / CLAUDE.md / `.mcp.json` / wrapper）有自己的 minor，反映文件 / skill 變動；Binary（GitHub release 端）有自己的 minor，反映 MCP server 內部新增 tool 或修 bug。Wrapper auto-download 從 release fetch binary 到 `~/bin/CheWordMCP`。
...
- **`ooxml-swift v0.19.0–0.19.1`** — `document.xml` lossless round-trip：root namespace preservation（34 `xmlns:*` + `mc:Ignorable`）+ Bookmark Reader parsing + Wrapper hybrid model（`Hyperlink` / `FieldSimple` / `AlternateContent` typed editable surface + raw passthrough）+ sort-by-position Writer emit + 6 raw-carrier types for `<w:p>` schema completeness。v0.19.1 修正 v0.19.0 的 pPr double-emission silent regression
