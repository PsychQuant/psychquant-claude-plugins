...
當前版本：**v3.17.3**（Plugin shell + Binary 同步）— bump `ooxml-swift` dep 0.21.6 → 0.21.7，exposes the canonical anchor-lookup API as `public` so external Swift SPM consumers (rescue scripts, dxedit CLI, third-party tooling) can call exactly what `insert_paragraph` / `insert_image_from_path` / `insert_caption` see when resolving `after_text` / `before_text` anchors. Pure transitive bump, no MCP source changes, backward compatible (private → public visibility only).
