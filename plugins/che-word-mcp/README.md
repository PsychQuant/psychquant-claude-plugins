...
當前版本：**v3.17.4**（Plugin shell + Binary 同步）— bump `ooxml-swift` dep 0.21.7 → 0.21.8，paired #91 + #92 release. #91 adds dedicated `InsertLocationError.inlineModeRequiresParagraphIndex` + `.invalidParagraphIndex(Int)` for `insert_equation` semantically-invalid anchor + out-of-bounds index (was silent fallthrough / no-op). #92 extends `flattenedDisplayText` OMML walk to hyperlink / fldSimple / AC.fallbackRuns wrapper paths so anchor lookups against paragraphs containing inline math inside those wrappers now succeed where they previously silently 0-matched. Pure transitive bump, no MCP source changes, backward compatible (strict superset of pre-fix behavior).
