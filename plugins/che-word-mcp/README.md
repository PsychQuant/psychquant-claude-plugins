...
當前版本：**v3.13.9**（Plugin shell + Binary 同步）

Office.js OOXML Roadmap **P0 100% 完成**（Umbrella issue [#43](https://github.com/PsychQuant/che-word-mcp/issues/43)）。Latest milestone v3.13.9 — sub-stack A-CONT-3 mini-cycle of [#58](https://github.com/PsychQuant/che-word-mcp/issues/58) ships ooxml-swift v0.19.9 closing 3 P0 from the [sub-stack A-CONT-2 6-AI verify](https://github.com/PsychQuant/che-word-mcp/issues/58#issuecomment-4323715199) **including a silent correctness regression** (delete_bookmark for header/footer bookmarks was lost on disk in v3.13.8 — must-upgrade for any v3.13.8 user calling delete_bookmark on container bookmarks). Plus list_bookmarks now surfaces paragraph-level container bookmarks (the more common real-world case); insert_bookmark detects duplicate names across all 5 part types. Sub-cycle 4 for #58 — same convergence-cycle pattern as R5→R5-CONT-4. v3.13.8 shipped sub-stack A-CONT-2; v3.13.7 sub-stack A-CONT; v3.13.6 sub-stack A initial fix。B re-numbered to v3.13.10 / [#59](https://github.com/PsychQuant/che-word-mcp/issues/59) whitespace overlay; C unchanged v3.14.0 / [#60](https://github.com/PsychQuant/che-word-mcp/issues/60) RunProperties audit。前次 milestone v3.13.5 R5 stack-completion — 詳見 [v3.13.5 closing summary](https://github.com/PsychQuant/che-word-mcp/issues/56#issuecomment-4322638865) 與 [CHANGELOG](https://github.com/PsychQuant/che-word-mcp/blob/main/CHANGELOG.md)。**ORIGINAL v3.13.7-v3.13.8 verify trail of partial-fixes deferred at sub-stack A-CONT-2 stage — see [verify report](https://github.com/PsychQuant/che-word-mcp/issues/58#issuecomment-4323715199) for what's deferred to follow-up SDD**。 ships ooxml-swift v0.19.8 closing 2 P0 + 1 P1 from the [sub-stack A-CONT 6-AI verify](https://github.com/PsychQuant/che-word-mcp/issues/58#issuecomment-4323658377): `getBookmarks()` now walks container `bodyChildren` across headers/footers/footnotes/endnotes (was body-only); `parseContainerChildBodyChildren` now has `case "sdt"` recursion mirroring `parseBodyChildren` (was missing — block-level SDTs in headers fell to `.rawBlockElement`); `deleteBookmark()` now symmetric with `getBookmarks()` (was: lists names it couldn't delete). Sub-cycle 3 for #58 (A → A-CONT → A-CONT-2). Same convergence-cycle pattern as R5→R5-CONT-4 (5 sub-cycles for #56) — each round catches what the prior matrix-pin couldn't see. v3.13.7 shipped sub-stack A-CONT (parser-side mirror); v3.13.6 shipped sub-stack A initial fix (BodyChild enum extension). B re-numbered to v3.13.9 / [#59](https://github.com/PsychQuant/che-word-mcp/issues/59) whitespace overlay; C unchanged v3.14.0 / [#60](https://github.com/PsychQuant/che-word-mcp/issues/60) RunProperties audit。前次 milestone v3.13.5 R5 stack-completion 30 findings closed across [#56](https://github.com/PsychQuant/che-word-mcp/issues/56) rounds 4-8 6-AI verify — 詳見 [v3.13.5 closing summary](https://github.com/PsychQuant/che-word-mcp/issues/56#issuecomment-4322638865) 與 [CHANGELOG](https://github.com/PsychQuant/che-word-mcp/blob/main/CHANGELOG.md)。
