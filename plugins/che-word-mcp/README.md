...
**Word MCP Server** — Swift 原生 OOXML 操作，**218+ 個工具**，支援 Dual-Mode 存取 + preserve-by-default round-trip fidelity + programmatic Track Changes 生成。

當前版本：**Plugin shell v3.13.0 / Binary v3.12.0**

Office.js OOXML Roadmap **P0 100% 完成**（Umbrella issue [#43](https://github.com/PsychQuant/che-word-mcp/issues/43)）。Latest milestone v3.12.0 ship programmatic Track Changes 寫側（[#45](https://github.com/PsychQuant/che-word-mcp/issues/45)）— 可生成 Word 原生可審閱的 `<w:ins>` / `<w:del>` / `<w:moveFrom>` / `<w:moveTo>` / `<w:rPrChange>` / `<w:pPrChange>` 修訂標記。
...
底層 `ooxml-swift v0.18.0` 採用 **preserve-by-default + dirty tracking** 架構：`open_document` 保留原始 archive tempDir；`save_document` overlay 模式透過 `WordDocument.modifiedParts: Set<String>` 精確判斷哪些 part 真正被改動，**未改動的 typed-managed part 完全不重寫**——byte-for-byte 保留 `word/theme/`、`webSettings.xml`、`people.xml`、`commentsExtended/Extensible/Ids`、`glossary/`、`customXml/`、**以及 `word/document.xml`、`styles.xml`、`fontTable.xml`、`header*.xml`、`footer*.xml`、`comments.xml`、`footnotes.xml`、`endnotes.xml`** 等所有 typed parts。
...
- **OOXML 引擎**: [`ooxml-swift v0.18.0`](https://github.com/PsychQuant/ooxml-swift)（preserve-by-default + dirty tracking + revision generation）
...
- **Plugin shell**: v3.13.0
- **Binary**: v3.12.0（`CheWordMCP`）
...
兩者獨立。Plugin shell（marketplace 端，含 SKILL.md / CLAUDE.md / `.mcp.json` / wrapper）有自己的 minor，反映文件 / skill 變動；Binary（GitHub release 端）有自己的 minor，反映 MCP server 內部新增 tool 或修 bug。Wrapper auto-download 從 release fetch binary 到 `~/bin/CheWordMCP`。

### 重要 milestones

- **v3.13.0** — Plugin shell 補齊：完整 SKILL.md（Direct/Session 雙模式、Track Changes 寫側合約、SDT 工作流）+ CLAUDE.md（Components / Architecture / Distribution / Office.js Roadmap closure map）。Binary 不變
