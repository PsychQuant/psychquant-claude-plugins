...
當前版本：**v3.13.6**（Plugin shell + Binary 同步）

Office.js OOXML Roadmap **P0 100% 完成**（Umbrella issue [#43](https://github.com/PsychQuant/che-word-mcp/issues/43)）。Latest milestone v3.13.6 bumps `ooxml-swift` to v0.19.6 + `word-to-md-swift` to v0.6.1，closes [#58](https://github.com/PsychQuant/che-word-mcp/issues/58)（sub-stack A of `che-word-mcp-issue-58-59-60-document-content-preservation`）— body-level `<w:bookmarkStart>` / `<w:bookmarkEnd>`（TOC `_Toc<digits>` 多段落跨段錨點）現在能在 body-mutating save 後完整保留。架構原則「**if not typed, preserve as raw**」— `BodyChild` enum 擴增 `.bookmarkMarker`（typed）+ `.rawBlockElement`（generic catch-all），同 `Run.rawElements`（v0.14.0+, #52）的 18 個月證明的 pattern。Cross-cutting matrix-pin `testDocumentContentEqualityInvariant` 加入 `<w:bookmarkStart>` count parity（45=45 thesis fixture）。B（v3.13.7 / [#59](https://github.com/PsychQuant/che-word-mcp/issues/59) whitespace overlay）+ C（v3.14.0 / [#60](https://github.com/PsychQuant/che-word-mcp/issues/60) RunProperties audit）後續發布。前次 milestone v3.13.5 R5 stack-completion 30 findings closed across [#56](https://github.com/PsychQuant/che-word-mcp/issues/56) rounds 4-8 6-AI verify — 詳見 [v3.13.5 closing summary](https://github.com/PsychQuant/che-word-mcp/issues/56#issuecomment-4322638865) 與 [CHANGELOG](https://github.com/PsychQuant/che-word-mcp/blob/main/CHANGELOG.md)。
